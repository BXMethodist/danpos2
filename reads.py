...
if __name__ == "__main__":
    ...
    rdf,genef,outname=sys.argv[1],sys.argv[2],sys.argv[1][:-3]
    print rdf.split('.')[-1]
    rd=reads(file=rdf,format=rdf.split('.')[-1])
    wg=rd.toWig(fs=150,extend=150)
    wg.foldChange(10000000*150/wg.sum())
    
    #tssup,tssdn=3500,3000
    for tssup in range(1000,10000,1000):
        for tssdn in range(1000,10000,1000):
            d={}
            for line in open(genef):
                col=line.split()
                col[3],col[4]=int(col[3]),int(col[4])
                if col[2]=='+':name,cr,start,end=col[0],col[1],col[3]-tssup,col[3]+tssdn
                elif col[2]=='-':name,cr,start,end=col[0],col[1],col[4]-tssdn,col[4]+tssup
                if not wg.data.has_key(cr):d[name]=0
                elif end>start:
                    d[name]=0
                    start,end=max(start/wg.step,0),min(end/wg.step,wg.data[cr].size-1)
                    for tstart in range(start,end-6000/wg.step,100/wg.step):
                        for tend in range(start+6000/wg.step,end,100/wg.step):
                            value=wg.data[cr][tstart:tend].sum()
                            if d[name]<value:d[name]=value#/(wg.data[cr][start:end].max()+1)
                elif end==start:d[name]=wg.data[cr][start]
                else:d[name]=0
            from operator import itemgetter
            fo=open(outname+'.tss.up'+str(tssup)+'dn'+str(tssdn)+'.xls','w')
            i=0
            for item in sorted(d.items(),key=itemgetter(1),reverse=True):
                i+=1
                fo.write('\t'.join(    [   item[0], str(item[1]),str(i)])+'\n')
    
    '''
    rdf2=sys.argv[4]

    
    rd2=reads(file=rdf2,format='bowtie')
    wg2=rd.toWig(fs=150,extend=150)
    wg2.foldChange(10000000*150/wg2.sum())
    ...
    for line in open(genef):
        ...
        start,end=max(start/wg.step,0),min(end/wg.step,wg.data[cr].size-1)
        d[name]=wg.data[cr][start:end].sum()
    d2={}
    for line in open(genef):
        col=line.split()
        col[3],col[4]=int(col[3]),int(col[4])
        ...
        start,end=max(start/wg2.step,0),min(end/wg2.step,wg2.data[cr].size-1)
        d2[name]=wg2.data[cr][start:end].sum()
    ...
    rk2={}
    i=1
    for item in sorted(d2.items(),key=itemgetter(1),reverse=True):
        rk2[item[0]]=i
        i+=1
        
    for item in sorted(d.items(),key=itemgetter(1),reverse=True):  
        fo.write('\t'.join(    [   item[0], str(item[1]), str(d2[item[0]]), str(item[1]-d2[item[0]]),str(rk[item[0]]),str(rk2[item[0]]) ,str(rk[item[0]]+rk2[item[0]])  ]   )+'\n')
    '''
    #python /archive/tmhkxc48/tools/danpos2.2.4/reads.py Keji_H3K27me3.bed /archive/tmhkxc48/broadH3K27me3/geneset/hg19.GREATgene2UCSCknownGenes.table.xls Keji_H3K27me3
    #python /archive/tmhkxc48/tools/danpos2.2.4/reads.py Nali_H3K27me3_rep1.20150402.bowtie /archive/tmhkxc48/broadH3K27me3/geneset/hg19.GREATgene2UCSCknownGenes.table.xls Nali_H3K27me3
