...
def occAroundPoints(wg,chrColID,nameColID,posColIDpos,posColIDneg,straColID,step=0,sep='\t',second_sep=None,\
    ...
    for line in lines:
        if line[:-1]=='\n':line=line[:-1]
        col=line.split(sep)
        try:chr,name,stra=col[chrColID],col[nameColID],col[straColID]
        except:
            print 'wrong line:',col
            continue
        if not wg.data.has_key(chr):
            print chr,'does not exist:',col
            continue
        if stra=='+':
            ...
            for bin in bins:
                ...
                for start in gd[cr][sel[0]][bin]:
                    for i in range(len(gd[cr][sel[0]][bin][start])):
                        ...
                        #if gd[cr][sel[0]][bin][start][i][0]=='+' (maxpos>start+sel[1] and minpos<end+sel[2]): # remove by kaifu on 2016/10/27
                        if gd[cr][sel[0]][bin][start][i][0]=='+':# add by kaifu on 2016/10/27
                            tstart,tend=min(start+sel[1],end),max(end+sel[2],start)
                            if tstart>tend:tstart,tend=int((tstart+tend)/2),int((tstart+tend)/2)
                            if (maxpos>tstart and minpos<tend): # add by kaifu on 2016/10/27
                                tgenes[gd[cr][sel[0]][bin][start][i][1]+'/'+str(start)+'/'+str(end)]=1
                        #elif gd[cr][sel[0]][bin][start][i][0]=='-' and (maxpos>start-sel[2] and minpos<end-s7yel[1]): # remove by kaifu on 2016/10/27
                        elif gd[cr][sel[0]][bin][start][i][0]=='-' :# add by kaifu on 2016/10/27
                            tstart,tend=min(start-sel[2],end),max(end-sel[1],start)
                            if tstart>tend:tstart,tend=int((tstart+tend)/2),int((tstart+tend)/2)
                            if (maxpos>tstart and minpos<tend):# add by kaifu on 2016/10/27
                                tgenes[gd[cr][sel[0]][bin][start][i][1]+'/'+str(start)+'/'+str(end)]=1
            if len(tgenes)!=0:
        ...
        for line in [line1,line2]:
            ...
            for i in range(len(t2)):
                if t2[i]=='M' :#all matched nucleotide will be counted into wiggle format data.
                    ...
                    allwig.data[col[2]][start/step:end/step]+=1.0
...
def overlap(afile,bfile,ofile,dis=0,pclarge=0,pcsmall=0,ovsz=0):
    fo=open(ofile+'.overlap.union.xls','w')
    fos=open(ofile+'.overlap.intersection.xls','w')
    foa=open(ofile+'.unique.'+afile,'w')
    foa.write(open(afile).readline())
    fob=open(ofile+'.unique.'+bfile,'w')
    fob.write(open(bfile).readline())
    fo.write('file_a: '+afile+'\n')
    fo.write('file_b: '+bfile+'\n')
    fo.write('chr\tunion_start\tunion_end\ta_start\ta_end\tb_start\tb_end\tintersection_size\tdistance\n')
    fos.write('file_a: '+afile+'\n')
    fos.write('file_b: '+bfile+'\n')
    fos.write('chr\tintersection_start\tintersection_end\ta_start\ta_end\tb_start\tb_end\tintersection_size\tdistance\n')
    d,a,b={},{},{}
    for line in open(afile).readlines()[1:]:
        col=line.split()
        try:chr,start,end=col[0],int(col[1]),int(col[2])
        except:continue
        a[chr+'|'+str(start)+'|'+str(end)]=line
        if not d.has_key(chr):d[chr]={}
        d[chr][start]=end
    overa,overb={},{}
    for line in open(bfile).readlines()[1:]:
        col=line.split()
        try:chr,bstart,bend=col[0],int(col[1]),int(col[2])
        except:continue
        b[chr+'|'+str(bstart)+'|'+str(bend)]=line
        if not d.has_key(chr):d[chr]={}
        for astart in d[chr]:
            aend=d[chr][astart]
            if bstart <aend+dis:
                if bend>astart-dis:
                    rdis=max(astart-bend,bstart-aend,0)
                    asize,bsize=aend-astart,bend-bstart
                    rovsz=min(bend-astart,aend-bstart,asize,bsize)+1
                    rpclarge=rovsz*100.0/max(asize,bsize)
                    rpcsmall=rovsz*100.0/min(asize,bsize)
                    if (rdis<=dis) or ((rpcsmall>pcsmall) and (rpclarge>pclarge) and  (rovsz>ovsz)):
                        if rovsz<1:rovsz=1
                        overa[chr+'|'+str(astart)+'|'+str(aend)]=1
                        overb[chr+'|'+str(bstart)+'|'+str(bend)]=1
                        ps=sorted([astart,aend,bstart,bend])                        
                        fo.write('\t'.join([chr,str(ps[0]),str(ps[-1]),str(astart),str(aend),str(bstart),str(bend),str(rovsz-1),str(rdis)])+'\n')
                        if rovsz>0:fos.write('\t'.join([chr,str(ps[1]),str(ps[-2]),str(astart),str(aend),str(bstart),str(bend),str(rovsz-1),str(rdis)])+'\n')
                        else:fos.write('\t'.join([chr,str(ps[1]),'NA','NA',str(aend),str(bstart),str(bend),str(rovsz-1),str(rdis)])+'\n')
    print afile,':',len(a),'total,',len(overa),'overlapped'
    print bfile,':',len(b),'total,',len(overb),'overlapped'
    for k in a:
        if not overa.has_key(k):foa.write(a[k])
    for k in b:
        if not overb.has_key(k):fob.write(b[k])
    print ''

def retrieveDNA(file,gfile,ofile):
    fo=open(ofile,'w')
    fi=open(file)
    tline=fi.readline()
    while(len(tline.split())<3):tline=fi.readline()
    d={}
    for line in fi:
        col=line.split()
        if col[1]!='NA':
            cr,start,end=col[0],int(col[1]),int(col[2])
            if not d.has_key(cr):d[cr]={}
            if d[cr].has_key(start):
                if end>d[cr][start]:d[cr][start]=end
            else:d[cr][start]=end
    
    cr,seq,start,end,starts='','',0,0,[]
    tc,tl=0,0
    for line in open(gfile):
        if line[0]=='>':
            print cr,start,end,tl,starts
            col=line[1:].split()
            cr=col[0]
            if d.has_key(cr):starts=d[cr].keys()
            else:starts=[]
            tc+=len(starts)
            starts.sort()
            start,end,seq=0,0,''
            ...
            if lth>0:
                ...
                while (lth>0):
                    tstart,tend=starts[0]-start,d[cr][starts[0]]-start
                    if (starts[0]>=start and d[cr][starts[0]]<=end):
                        fo.write('>'+cr+'_'+str(starts[0])+'_'+str(d[cr][starts[0]])+'\n'+seq[tstart:tend]+'\n')
                        #if lth<2:print '>'+cr+'_'+str(starts[0])+'_'+str(d[cr][starts[0]])+'\n'+seq[tstart:tend]
                        starts=starts[1:]
                        lth-=1
                    else:
                        lth=0
