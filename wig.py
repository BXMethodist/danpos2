...
class Wig:
    ...
    def bgsub(self,wig2,lmd=1000):
        ...
        for chr in chrs:
            ...
            if not wig2.data.has_key(chr):wig2.data[chr]=numpy.array([0.0])
    ...
    def callSlopes(self,width=40,swidth=40,mstep=10,height=0,regions=None,ofile='test_w40_3.bed',slopecut=0.1, large_drop=5):
        '''
        Description:
            call occupancy slopes using a sliding window.
        
        Parameter:
            width: the width of the sliding window used to call slopes.
            swidth: the cutoff for minimal width of a slope
            mstep: the moving step of the sliding window
            height: the cutoff for the occupancy difference between neighbor sliding windows.. valide only when pcut is set to 1.
            slopecut: the cutoff for minimal slope value (occupancy difference divided by distance from start point to end point).
            large_drop: the cutoff for the large drops that are to be reported.
            regions: A set of predefined regions in which the summits are to be defined, regions[chromatin_name][start_position]=end_position
        '''
        step=self.step
        mstep=max(mstep/step,1)
        width=width/step
        swidth=swidth/step
        fo=open(ofile,'w')
        ...
        if regions==None:
            ...
            for cr in self.data:
                regions[cr]={}
                regions[cr][0]=(self.data[cr].size-1)*step
        for cr in regions:#self.data:
            ...
            for p in region_poses:
                ...
                if dlth>sz:dlth=sz
                if i+width>dlth:continue
                vi=lst[i:(i+width)].max()
                slop_dire=0 #the direction of slop, 1==up,0==flat,-1==down
                i+=mstep
                while i<dlth:
                    v=lst[i:(i+width)].max()
                    if v+height<vi:
                        if slop_dire==0:#switch from flat to slope down
                            start=i-mstep
                        elif slop_dire==1:#switch from slope up to slope down
                            if end-start>=swidth:
                                slope=abs(lst[end]-lst[start])/((end-start)*step)
                                drop_positions, drop_values=[],[]
                                for k in range(start,end):
                                    drop=lst[k+1]-lst[k]
                                    if abs(drop)>=large_drop:drop_positions.append(str(k*step)),drop_values.append(str(drop))
                                if len(drop_positions)==0:
                                    drop_positions.append('None')
                                    drop_values.append('None')
                                if slope>=slopecut: fo.write('\t'.join([cr, str(start*step),str(end*step),str(lst[start]),str(lst[end]),'+',str((end-start)*step),str(slope),','.join(drop_positions),','.join(drop_values)])+'\n')
                            num+=1
                            start=i-mstep
                        slop_dire=-1
                        end=i+width/2
                    elif v>vi+height:
                        if slop_dire==0:#switch from flat to slope up
                            start=i-mstep+width/2
                        elif slop_dire==-1:#switch from slope down to slope up
                            if end-start>=swidth:
                                slope=abs(lst[end]-lst[start])/((end-start)*step)
                                drop_positions, drop_values=[],[]
                                for k in range(start,end):
                        ...
                        if slop_dire!=0: #switch from slope to flat
                            if end-start>=swidth:
                                slope=abs(lst[end]-lst[start])/((end-start)*step)
                                drop_positions, drop_values=[],[]
                                for k in range(start,end):
    ...
    def callSummits(self,width=40,pcut=1,height=5,regions=None):
        ...
        return smts
    def changeStep(self,step,method='sum'):
        '''
        Description:
        ...
        Parameter:
            ...
            method: the method to define new value at each step, can be either max, sum, or mid
        ...
        if step>self.step:
            ...
            for chr in self.data:
                ...
                for npos in range(0,nlth):
                    ...
                    elif method=='max':lst[npos]+=self.data[chr][(npos*fd):((npos+1)*fd)].max()
                    elif method=='mid':
                        midpos=((npos*fd)+((npos+1)*fd))/2
                        lst[npos]+=self.data[chr][midpos]
                    else:
                        print "the method should be either 'sum' or 'max' and can not be the \'"+method+"\'specified here!"
                        return false
                self.data[chr]=lst
                ...
                if chr != '':
                    ...
                    if instep!=self.step:
                        ...
                        for pos in range(len(lst)):
                            if instep<self.step:
                                try:nplst[pos*instep/self.step]+=lst[pos] ########## add by kaifu on Sep 6, 2012 ##########
                                except: continue
                            else:
                                for tpos in range(pos*instep/self.step,(pos+1)*instep/self.step):
                                    try:nplst[tpos/self.step]+=lst[pos]*1.0*self.step/instep ########## add by kaifu on Sep 6, 2012 ##########
                                    except:continue
